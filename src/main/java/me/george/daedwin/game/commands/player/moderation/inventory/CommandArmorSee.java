package me.george.daedwin.game.commands.player.moderation.inventory;
...
public class CommandArmorSee implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        p.sendMessage(ChatColor.GREEN + "Accessing armor for " + ChatColor.YELLOW + target.getName());

        Inventory inventory = target.getInventory();
        ItemStack[] armorContents = ((PlayerInventory) inventory).getArmorContents();

        Inventory newInventory = Bukkit.createInventory(p, 9, ChatColor.RED + target.getName() + "'s Armor");
